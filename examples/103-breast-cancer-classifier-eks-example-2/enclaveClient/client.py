...
def main():
    ...
    print(f"Received msg from Server: {response}. You can write to S3.")

    key = uuid.uuid4()
    tmp_result_file = f"/tmp/{key}"
    with open(tmp_result_file) as file:
        file.write(response)

    # Upload to S3
    s3_client = boto3.client('s3')
    s3_object_name = f'inference_result_{key}.txt'
    try:
        response = s3_client.upload_file(tmp_result_file, os.getenv('RESULT_UPLOAD_BUCKET'),
                                         s3_object_name)
        logging.info(
            "Uploaded result to s3://{}/{}".format(os.getenv('RESULT_UPLOAD_BUCKET'), s3_object_name))
